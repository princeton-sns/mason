...
template <class TTr>
bool Rpc<TTr>::reset_session(int session_num) {
    Session *session = session_vec[static_cast<uint16_t>(session_num)];
    printf("reset_session resetting sn %d -> lsn %d\n", session_num, session->local_session_num);
    bool ret = handle_reset_client_st(session);
    printf("reset_session about to return\n");
    return ret;
}
