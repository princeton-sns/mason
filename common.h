...
enum class ReqType : uint8_t {
  ...
  /// Triggers watches left by kExists (kNodeRenamed) on new/old server,
  /// kGetchildren (kNodeChildrenChanged) on parent,
  /// kReadZNode kExists kNodeRenamed on old server
  kRenameZNode,
...
// ok, almost all of these are the same...
// payloads
...
enum CreateNodeOptions : uint8_t {
  kPersistent, // default
  kPersistentSequential, // todo
  kEphemeral,
  kEphemeralSequential, // todo
};
...
// added to get unique client ID for the proxy to delete
uint16_t
...
#define SEQUENCE_PAYLOAD_SIZE(n) (sizeof(payload_t) - sizeof(seq_req_t) + n*sizeof(seq_req_t))
// Seq payload
