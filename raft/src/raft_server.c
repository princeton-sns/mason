...
void raft_become_follower(raft_server_t* me_)
{
    ...
    // only reset if I was not a follower before
...
//        __log(me_, NULL, "I am leader never starting election");
    }
             ...
             !raft_snapshot_is_in_progress(me_))
    {
            ...
            raft_node_is_voting(raft_get_my_node(me_)))
        {
            ...
            __log(me_, NULL, "I am not a voting node, not starting election");
        }
    } //else {
//        if (raft_get_current_leader(me_) == -1)
//            __log(me_, NULL, "no leader but did not timeout (elapsed %d since last %d rand timeout %d,"
//                             " not starting election or snapshot in progress: %d",
//                             me->timeout_elapsed, msec_since_last_period, me->election_timeout_rand,
//                             raft_snapshot_is_in_progress(me_));
//    }
...
static int __should_grant_vote(raft_server_private_t* me, msg_requestvote_t* vr)
{
    if (!raft_node_is_voting(raft_get_my_node((void*)me))) {
        printf("rejecting leader because I am not a voting node\n");
        return 0;
    }

    if (vr->term < raft_get_current_term((void*)me)) {
        printf("rejecting leader because our term is greater\n");
        return 0;
    }

    /* TODO: if voted for is candidate return 1 (if below checks pass) */
    if (raft_already_voted((void*)me)) {
        printf("rejecting leader because I already voted for this term\n");
        return 0;
    }
    ...
    if (ety)
        ...
        printf("rejecting leader because me->snapshot_last_idx != current_idx\n");
        return 0;
    }
    ...
    printf("rejecting leader because my log is more up-to-date\n");
    return 0;
}
                          ...
                          msg_requestvote_response_t *r)
{
    ...
    if (me->current_leader && me->current_leader != node &&
        ...
        printf("rejecting leader because we already have a leader\n");
        r->vote_granted = 0;
        goto done;
    }
    ...
    if (raft_get_current_term(me_) < vr->term)
    {
        ...
        if (0 != e) {
            r->vote_granted = 0;
            printf("rejecting because we could not set current term\n");
            goto done;
        }
        raft_become_follower(me_);
        me->current_leader = NULL;
