...
class RecoveryContext : public ThreadContext {
 public:
  ...
  // points to the same SequenceSpaces given to WorkerContext
  ...
  size_t tags[MAX_PROXIES]; // machine idx
...
void
run_recovery_loop(erpc::Nexus *nexus,
                  size_t nleaders,
                  SequenceSpaces *_sequence_spaces) {
  auto *c = new RecoveryContext;

  c->sequence_spaces = _sequence_spaces;
...
uint64_t
find_next_hole(bool reset_static_vars) {
  ...
  if (reset_static_vars) {
    next_seqnum = agg_seqnums->nblocks * SEQNUMS_PER_BLOCK +
        agg_seqnums->base_seqnum;
    last_assigned_seqnum = 0;
    return 0;
  }
  ...
  debug_print(DEBUG_RECOVERY, "found byte: %"
  PRIu8
  ", idx %zu\n",
      bitmap[idx], idx);
  for (; bit < 8; bit++) {
    if (!(bitmap[idx] & (1 << bit))) {
      ...
      bitmap[idx] |= (1 << bit);
      debug_print(DEBUG_RECOVERY, "found the hole: min %"
  ...
  // this should never execuate. if the byte is not 0xff == 0b11111111
  // then there is a number to be sent?
  // todo after 15034615 needed to return default
  debug_print(DEBUG_RECOVERY, "find_next_hole returning default: byte was %"
              PRIu8 "\n", bitmap[idx]);
...
void
request_bitmap(RecoveryContext *c,
               size_t pid,
               uint64_t offset,
               uint64_t sequence_space) {
  debug_print(DEBUG_RECOVERY, "Requesting bitmap at offset %lu, pid %zu\n",
              offset, pid);
  auto *payload = reinterpret_cast<offset_payload_t *>(
      c->req_msgbufs[pid].buf);
  payload->offset = offset;
  payload->sequence_space = sequence_space;
...
request_dependencies(RecoveryContext *c) {
  ...
  for (size_t i = 0; i < other_ip_list.size(); i++) {
    if (c->active_proxies[i]) {
      ...
      debug_print(DEBUG_RECOVERY,
                  "%zu's number of outstanding requests is %d\n",
                  i,
                  nrequests);
      ...
      if (nrequests > 0) {
        ...
        c->batch_requests +=
            nrequests;  // Keep track of #batches needing seqnums
      ...
      debug_print(DEBUG_RECOVERY,
                  "%zu is not an active proxy! Skipping...\n", i);
    }
  }
}
...
void
process_bitmap_payload(RecoveryContext *c,
  ...
  if (c->recovery_data[i] == nullptr) {
    ...
    c->recovery_data[i]->session_num =
        static_cast<size_t>(c->session_num_vec.at(i));
    c->recovery_data[i]->nblocks_received = 0;
    c->recovery_data[i]->nrequests =
        resp->nrequests;  // Batches that need seqnums
    c->recovery_data[i]->nblocks_total = resp->nblocks_total;

    auto *received_seqnums = new Bitmap;
...
request_bitmap_cont_func(void *_context, void *_tag) {
  ...
  uint64_t sequence_space =
      reinterpret_cast<recovery_payload_t *>(resp_msgbuf->buf)->sequence_space;
  ...
  if (c->recovery_data[i]->nblocks_received <
    ...
    request_bitmap(c, i, offset, sequence_space);
    return;
  }
...
process_proxy_bitmap(RecoveryContext *c, size_t machine_idx) {
  ...
  if (DEBUG_RECOVERY) {
    debug_print(DEBUG_RECOVERY, "Metadata is: base: %"
    PRIu64
    " "
  ...
  if (agg_seqnums == nullptr) {
    ...
    debug_print(DEBUG_RECOVERY,
                "Agg head: %zu, tail %zu, base_seqnum %zu; "
                "\n\tNew head %zu, tail %zu, base_seqnum %zu\n",
                agg_seqnums->head_block,
                agg_seqnums->tail_block,
                agg_seqnums->base_seqnum,
                p_seqnums->head_block,
                p_seqnums->tail_block,
                p_seqnums->base_seqnum);
...
process_dependency_payload(RecoveryContext *c, erpc::MsgBuffer *resp_msgbuf,
  ...
  auto *dep_array = reinterpret_cast<dependency_t *>(resp_msgbuf->buf);

  size_t ndeps = c->recovery_data[i]->nrequests;
  erpc::rt_assert(
      resp_msgbuf->get_data_size() == (ndeps * sizeof(dependency_t)),
...
uint64_t
assign_seqnums_to_holes_and_send(RecoveryContext *c,
                                 dependency_t *deps,
                                 uint64_t sequence_space) {
  uint64_t hole = 0;
  ...
  for (size_t i = 0; i < c->batch_requests; i++) {
    ...
    for (size_t j = 0; j < deps[i].batch_size; j++) {
      ...
      hole = find_next_hole(false);

      // why is this called response?
      ...
      response->sequence_space = sequence_space;
...
assign_seqnum_to_hole_cont_func(void *_c, void *_tag) {
  (void) _tag;
  auto *c = reinterpret_cast<RecoveryContext *>(_c);
  c->hole_acks++;
}
...
check_bitmap_for_holelessness(uint64_t highest_assigned_seqnum) {
  ...
  temp = idx;
  debug_print(DEBUG_RECOVERY, "at start idx: %"
  PRIu64
  " temp: %"
  PRIu64
  "\n", idx, temp);
  ...
  for (;;) {
    if (foundFirstZero && (bitmap[idx] != 0x00)) {
      ...
      printf("Found a hole... idx: %"
      PRIu64
      " %"
      PRIu64
      " "
      "Dying...\n", idx, sequence_number);
      printf("the final byte is %"
      PRIu8
      "\n", bitmap[idx]);
        ...
        if (!(bitmap[idx] & (1 << i))) {
          if (!foundFirstZero) {
            ...
            printf("The next sequence number to assign is "
                   "%" PRIu64 "\n", sequence_number);
            debug_print(DEBUG_RECOVERY, "Found first zero at idx: %" PRIu64
                        " %" PRIu64 "\n", idx, sequence_number);
            foundFirstZero = true;
          }
        } else {
...
confirm_recovery_cont_func(void *_c, void *_tag) {
  ...
  debug_print(DEBUG_RECOVERY,
              "Got confirmation of recovery completion "
...
void RecoveryContext::reset_recovery_state() {
  proxy_responses = 0;
  hole_acks = SIZE_MAX;  // Got ACK of assigned seqnum in hole-filling
  nleaders = SIZE_MAX;
  nleaders_confirmed = 0;
  batch_requests = 0;
  total_holes = 0;

  // dependencies
  dependencies.clear();
  // reset recovery_data array
  for (int i = 0; i < MAX_PROXIES; i++) {
    delete recovery_data[i];
    recovery_data[i] = nullptr;
  }

  // delete and set agg_seqnums to null ptr
  delete agg_seqnums;
  agg_seqnums = nullptr;
...
recover(RecoveryContext *c) {
  ...
  // loop through recovering for each sequence space
  // this seems like it may be the easiest
  // but not fastest way to recover with MS
  for (size_t sequence_space = 0;
       sequence_space < c->sequence_spaces->nsequence_spaces;
       sequence_space++) {
    ...
    for (size_t i = 0; i < other_ip_list.size(); i++) {
      if (c->active_proxies[i]) {
        request_bitmap(c, i, 0, sequence_space);
      }
    }
    ...
    uint64_t highest_assigned_seqnum =
        assign_seqnums_to_holes_and_send(c, deps, sequence_space);
    ...
    debug_print(DEBUG_RECOVERY,
                "Proxies were notified of all holes for sequence_space %zu\n",
                sequence_space);

    // this points to sequence_spaces in Context
    c->sequence_spaces->sequence_spaces[sequence_space] =
        check_bitmap_for_holelessness(highest_assigned_seqnum);

    c->reset_recovery_state();
  }

  // confirm recovery after each sequence space has recovered
  confirm_recovery(c);
