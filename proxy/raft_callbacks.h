...
#define FALLING_BEHIND_WARNING 600000
...
void print_entry(entry_t *entry) {
    printf("type: %d seq_req_id %lu seq_num %lu batch_id %lu batch_size %u\n",
           static_cast<int>(entry->type), entry->seq_req_id,
           entry->seq_num, entry->batch_id, entry->batch_size);
    fflush(stdout);
}
...
struct app_appendentries_t {
    ...
    static inline void serialize(erpc::MsgBuffer &req_msgbuf, int node_id, int proxy_id,
                          msg_appendentries_t *msg_ae) {
        ...
        for (size_t i = 0; i < static_cast<size_t>(msg_ae->n_entries); i++) {
            ...
            *reinterpret_cast<msg_entry_t *>(buf) = msg_ae->entries[i];
            ...
            // Copy the entry data
            memcpy(buf, msg_ae->entries[i].data.buf, msg_ae->entries[i].data.len); //sizeof(client_req_t));
            buf += msg_ae->entries[i].data.len; // sizeof(client_req_t);
        }
    ...
    static inline void unpack(const erpc::MsgBuffer *req_msgbuf,
        ...
        if (!is_keepalive) {
            ...
            for (size_t i = 0; i < n_entries; i++) {
                ...
                memcpy(msg_ae.entries[i].data.buf, buf, msg_ae.entries[i].data.len);
...
// Return a string representation of a requestvote response message
...
inline int __send_requestvote(
    ...
    printf("trying to become the leader\n"); fflush(stdout);

    // receiver data
    auto *data = static_cast<replica_data_t *>(raft_node_get_udata(node));

    // If not connected don't send
    if (!wc->rpc->is_connected(wc->session_num_vec[static_cast<int>(data->idx)])) {
        return 0;
    }
    ...
    rv_tag->req_msgbuf = proxy->c->rpc->alloc_msg_buffer_or_die(sizeof(app_requestvote_t));
    rv_tag->resp_msgbuf = proxy->c->rpc->alloc_msg_buffer_or_die(sizeof(app_requestvote_t));
    ...
    printf("sending request_vote to session number: %d pid %d \n\t%s\n",
            proxy->c->session_num_vec[static_cast<int>(data->idx)], proxy->proxy_id,
            msg_requestvote_string(&rv_req->msg_rv).c_str());
    fflush(stdout);

    // enqueue it to be sent
    proxy->c->rpc->enqueue_request(proxy->c->session_num_vec[static_cast<int>(data->idx)],
...
inline void requestvote_handler(erpc::ReqHandle *req_handle, void *_context) {
    WorkerContext *wc = static_cast<WorkerContext *>(_context);
    ...
    LOG_INFO("Proxy id %d received requestvote, am I leader? %d\n",
             proxy->proxy_id, was_leader);

    int e = raft_recv_requestvote(proxy->raft,
                                  raft_get_node(proxy->raft, rv_req->node_id),
                                  &rv_req->msg_rv, rv_resp);

    LOG_INFO("Proxy id %d leader? %d\n", proxy->proxy_id, raft_is_leader(proxy->raft));

    erpc::rt_assert(e == 0, "error receiving requestvote %s\n", strerror(e));

    LOG_RAFT_SMALL("Proxy id %d sending requestvote response %s\n",
            proxy->proxy_id, msg_requestvote_response_string(rv_resp).c_str());

    wc->rpc->enqueue_response(req_handle, &req_handle->pre_resp_msgbuf);

    if (was_leader) {
        if (!raft_is_leader(proxy->raft)) {
            LOG_INFO("I JUST LOST LEADERSHIP for %d\n", proxy->proxy_id);
            proxy->lose_leadership();
        }
    }
}

// todo(cleanup) replace all proxy->raft with a variable "raft"...
...
inline void requestvote_cont(void *, void *_tag) {
    ...
    Proxy *proxy = rv_tag->proxy;

    printf("in request vote cont\n");
    fflush(stdout);
    ...
    if (likely(rv_tag->resp_msgbuf.get_data_size() > 0)) {
        ...
        printf("smr: %s Received requestvote response: %s\n", proxy->p_string().c_str(),
                msg_requestvote_response_string(msg_rv_resp).c_str());
        fflush(stdout);

        int e = raft_recv_requestvote_response(proxy->raft, rv_tag->node, msg_rv_resp);
...
inline int __send_appendentries(
    ...
    auto *data = static_cast<replica_data_t *>(raft_node_get_udata(node));
     ...
     (void) is_keepalive; // there has to be a better way to avoid "unused" when logging is off

    if (data->nOutstanding >= MAX_OUTSTANDING_AE) {
        return 0;
    }

    LOG_RAFT_SMALL("smr: Sending appendentries (%s) to node_id %d proxy_id %d [%s].\n",
                is_keepalive ? "keepalive" : "non-keepalive",
                data->node_id, data->pid, erpc::get_formatted_time().c_str());
    ...
    if (proxy->am_leader && data->node_id == 2) {
        return 0;
    }
    ...
    for (size_t i = 0; i < static_cast<size_t>(msg->n_entries); i++) {
        ...
        if (req_size + sizeof(msg_entry_t) + msg->entries[i].data.len > MAX_AE_SIZE) {
            if (i != 0) {
                msg->n_entries = i;
                break;
            }
    ...
    auto *log = reinterpret_cast<my_log_private_t *>((reinterpret_cast<raft_server_private_t *>(proxy->raft))->log);
    LOG_RAFT_SMALL("\t to node_id %d size of log %ld\n", data->node_id, reinterpret_cast<my_log_private_t *>((reinterpret_cast<raft_server_private_t *>(proxy->raft))->log)->count);
    LOG_RAFT_SMALL("\t to node_id %d prev_log_idx %ld n_entries %d appendentries size: %lu\n", data->node_id, msg->prev_log_idx, msg->n_entries, req_size);

    if (unlikely(log->count - msg->prev_log_idx > FALLING_BEHIND_WARNING)) {
        LOG_WARN("[%s] Thread %zu WARNING: REPLICA %d FELL OVER %d BEHIND! %ld BEHIND\n",
                erpc::get_formatted_time().c_str(), proxy->c->thread_id, data->node_id, FALLING_BEHIND_WARNING, log->count - msg->prev_log_idx);
    }

    // todo should be safe to remove this now
    ...
    // apparently this can fire near the end of the experiment
    erpc::rt_assert(req_size <= proxy->c->rpc->get_max_msg_size(),
                    "appendentries request size larger than max msg size of eRPC (should never fire)\n");

    raft_tag_t *rrt = proxy->raft_tag_pool.alloc();
    rrt->req_msgbuf = proxy->c->rpc->alloc_msg_buffer_or_die(req_size);
    rrt->resp_msgbuf =
            proxy->c->rpc->alloc_msg_buffer_or_die(sizeof(msg_appendentries_response_t));
    rrt->node = node;
    rrt->proxy = proxy;

    app_appendentries_t::serialize(rrt->req_msgbuf, proxy->replica_data[my_raft_id].node_id, proxy->proxy_id, msg);
    ...
    LOG_RAFT("enqueued appendentries!\n");
    data->nOutstanding++;
...
inline void appendentries_handler(erpc::ReqHandle *req_handle, void *_context) {
    ...
    LOG_RAFT_SMALL("smr: received appendentries from node_id %d for proxy_id %d prev_log_idx %ld n_entries %d  [%s]\n",
                node_id, proxy->proxy_id, msg_ae.prev_log_idx, msg_ae.n_entries, erpc::get_formatted_time().c_str());

    raft_tag_t *rrt = proxy->raft_tag_pool.alloc();

    rrt->req_msgbuf = proxy->c->rpc->alloc_msg_buffer_or_die(sizeof(ae_response_t));
    rrt->resp_msgbuf = proxy->c->rpc->alloc_msg_buffer_or_die(8); // unnecessary?
    rrt->node = raft_get_node(proxy->raft, proxy->replica_data[my_raft_id].node_id);
    rrt->proxy = proxy;
    ...
    response->bitmap_base_seqnum = proxy->c->received_seqnums->base_seqnum;
    ...
    auto session_num = proxy->c->session_num_vec[static_cast<int>(proxy->replica_data[node_id].idx)];
    ...
    proxy->c->rpc->enqueue_request(session_num, static_cast<uint8_t>(ReqType::kAppendEntriesResponse),
                                   &rrt->req_msgbuf, &rrt->resp_msgbuf,
                                   appendentries_response_cont, reinterpret_cast<void *>(rrt));
...
inline void appendentries_response_handler(erpc::ReqHandle *req_handle, void *_context) {
#if PRINT_TIMING
    auto start = erpc::get_formatted_time();
#endif

    auto start_time = erpc::get_formatted_time();
    auto *response = reinterpret_cast<ae_response_t *>(req_handle->get_req_msgbuf()->buf);
    ...
    if (!raft_is_leader(proxy->raft)) {
        printf("I am not the leader receiving an appendentries response\n");// I am %d the leader is %d\n",
        fflush(stdout);
    }
    ...
    LOG_RAFT_SMALL("proxy_id %u appendentries_response_handler about to queue response started [%s] now [%s]\n",
            proxy->proxy_id, start_time.c_str(), erpc::get_formatted_time().c_str());
    proxy->c->rpc->enqueue_response(req_handle, &req_handle->pre_resp_msgbuf);

    if (likely(req_handle->get_req_msgbuf()->get_data_size() > 0)) {
       LOG_RAFT_SMALL("smr: proxy_id %u received an appendentries response from node_id %d current idx %ld [%s]\n",
                   proxy->proxy_id, node_id, response->ae_response.current_idx, erpc::get_formatted_time().c_str());
        ...
        if (e == RAFT_ERR_NOT_LEADER) {
            printf("[%s] proxy_id %u recv_ae_response not the leader, calling raft_periodic leader is: %d\n",
                   erpc::get_formatted_time().c_str(), proxy->proxy_id, raft_get_current_leader(proxy->raft));
        ...
        LOG_RAFT("commit idx after receiving response: %lu matchindex: %lu my matchindex: %lu\n",
                raft_get_commit_idx(proxy->raft), raft_node_get_match_idx(raft_get_node(proxy->raft, node_id)),
...
void fix_client_pointers(Proxy *proxy,
        std::unordered_map<uint16_t, std::unordered_map<client_reqid_t, ClientOp *> > *m){
    for (auto p0 : *m) {
...
// boost changes the value of the pointer
// so the caller maintains a pointer to the old proxy
// need to return the pointer to the new proxy struct
inline Proxy *process_snapshot(WorkerContext *wc, snapshot_request_t *sr, Proxy *proxy) {
    auto time = erpc::get_formatted_time();
    LOG_INFO("Completed receiving the snapshot, updating proxy and deleting idx %ld term %ld\n",
            sr->last_included_idx, sr->last_included_term);

    // first we write the snapshot from memory to disk
    std::string filename = "/usr/local/backup_snapshot" + wc->my_ip + std::to_string(wc->thread_id) +
           std::to_string(proxy->proxy_id);
    std::string bm_filename = "/usr/local/backup_bitmap" + wc->my_ip + std::to_string(wc->thread_id) +
               std::to_string(proxy->proxy_id);

    FILE *ss_file = fopen(filename.c_str(), "w");
    fmt_rt_assert(fwrite(sr->snapshot, 1, sr->size_before_bm, ss_file) == sr->size_before_bm,
            "did not write all data?\n");
    fclose(ss_file);

    FILE *bm_file = fopen(bm_filename.c_str(), "w");
    fmt_rt_assert(fwrite(sr->snapshot+sr->size_before_bm, 1, sr->size - sr->size_before_bm, bm_file)
                   == sr->size - sr->size_before_bm,
                  "did not write all data?\n");
    fclose(bm_file);
    ...
    fmt_rt_assert(e == 0, "error beginning load snapshot idx %ld term %ld error %d\n",
    ...
    proxy->raft = raft;
    wc->received_seqnums = proxy->received_seqnums;

    // we now need to read in the bitmap file
    wc->received_seqnums->read_from_file(bm_filename.c_str(), sr->size - sr->size_before_bm);
    ...
    LOG_INFO("Finished loading snapshot last included index %ld proxy %p\n",
             proxy->last_included_index, static_cast<void *>(proxy));
...
inline void send_snapshot_handler(erpc::ReqHandle *req_handle, void *_context) {
    ...
    LOG_COMPACTION("received snapshot node_id, %d proxy_id, %d, rpc_n %zu msg_n %zu n_msgs %zu total size %zu"
                   " offset %zu length %zu\n",
                ss_req->node_id, ss_req->proxy_id,
                ss_req->rpc_n, ss_req->msg_n, ss_req->total_msg_size,
                ss_req->total_snapshot_size, ss_req->offset, ss_req->length);
    ...
    if (sr->msgs_rcvd == 0) {
        ...
        sr->msgs = ss_req->total_msg_size + 1;
        sr->size_before_bm = ss_req->size_before_bm;
        sr->size = ss_req->total_snapshot_size;
        sr->snapshot = static_cast<char *>(malloc(ss_req->total_snapshot_size));
    }

    // eRPC guarantees exactly-once
    sr->msgs_rcvd++;
    memcpy(sr->snapshot + ss_req->offset, &ss_req->snapshot_block, ss_req->length);
...
inline void send_snapshot_cont(void *, void *_tag) {
    raft_tag_t *tag = reinterpret_cast<raft_tag_t*>(_tag);
...
inline int __send_snapshot(
    ...
    (void)raft; (void)node; (void)user_data;

    // todo vary this?
    // only send once every 5 seconds
    // even with sending this slow many of the enqueued request (like half)
    // don't receive responses...
    static uint64_t last_time_sent = 0;
    static uint64_t freq_ghz = erpc::measure_rdtsc_freq(); // this is only done once... 12us
    uint64_t now = erpc::rdtsc();
    if (now - last_time_sent < erpc::us_to_cycles(5000000, freq_ghz)) {
        return 0;
    }
    else
        last_time_sent = erpc::rdtsc();
    ...
    std::string ss_file_name = "/usr/local/snapshot" + wc->my_ip + std::to_string(wc->thread_id) +
           std::to_string(proxy->proxy_id);
    std::string bm_file_name = "/usr/local/bitmap" + wc->my_ip + std::to_string(wc->thread_id) +
                   std::to_string(proxy->proxy_id);
    ...
    FILE *bm_file = fopen(bm_file_name.c_str(), "rb");
    fseek(bm_file, 0, SEEK_END);

    auto ss_size = static_cast<size_t>(ftell(ss_file));
    fseek(ss_file, 0, SEEK_SET);
    auto bm_size = static_cast<size_t>(ftell(bm_file));
    fseek(bm_file, 0, SEEK_SET);

    auto total_size = ss_size + bm_size;
    char *snapshot = reinterpret_cast<char *>(malloc(total_size));
    if (fread(snapshot, 1, ss_size, ss_file)) printf("read file\n");
    if (fread(snapshot+ss_size, 1, bm_size, bm_file)) printf("read bitmap file\n");
    fclose(ss_file); fclose(bm_file);
    ...
    while(size_left > 0) {
        auto msg_size = std::min(proxy->c->rpc->get_max_msg_size(), sizeof(send_snapshot_t) + size_left - sizeof(char *));
        ...
        ss_tag->resp_msgbuf = proxy->c->rpc->alloc_msg_buffer_or_die(sizeof(snapshot_request_t));

        fmt_rt_assert(ss_tag->req_msgbuf.buf != nullptr && ss_tag->resp_msgbuf.buf != nullptr,
                "msgbuffer was null\n");
        ...
        ss_req->offset = static_cast<size_t>(curr - snapshot);
        memcpy(&ss_req->snapshot_block, curr, length);
        curr += length;
        size_left -= length;
...
inline int __persist_vote(
    ...
    (void) raft; (void) user_data; (void) vote;
...
inline int __persist_term(
    ...
    (void) raft; (void) user_data; (void) term; (void) vote;
...
inline int __applylog(
    ...
    auto *our_entry = reinterpret_cast<entry_t *>(entry->data.buf);
    auto *my_data = static_cast<replica_data_t *>(user_data);
    auto *wc = my_data->wc;
    Proxy *proxy = wc->proxies[my_data->pid];
    ...
    switch(our_entry->type) {
        ...
        case EntryType::kDependency:
            erpc::rt_assert(false, "should not be in kDependency");
            break;

        // I just replicated a sequence number
        case EntryType::kSequenceNumberNoncontig:
        case EntryType::kSequenceNumber: 
            {
                debug_print(DEBUG_RAFT, "committing type kSequenceNumber%s\n",
                        our_entry->type == EntryType::kSequenceNumber? "" : "Noncontig");
                ...
                while (our_entry->base_seqnum > wc->received_seqnums->base_seqnum) {
                    wc->received_seqnums->mutex.lock();
                    wc->received_seqnums->truncate();
                    wc->received_seqnums->mutex.unlock();
                }

                // this was already done if I am the leader
                // but not if I was a follower when this was added to the log
                if (our_entry->type == EntryType::kSequenceNumberNoncontig) {
                    auto *cmdata =
                        reinterpret_cast<client_mdata_nc_t *>(&our_entry->cmdata_buf);
                    for (size_t i = 0; i < batch->batch_size(); i++) {
                        ClientOp *op = batch->batch_client_ops[i];
                        op->seqnum = cmdata[i].seqnum;
                ...
                erpc::exit_assert(our_entry->batch_size == batch->batch_size(),
                                  "batch sizes aren't the same!?\n");

                debug_print(DEBUG_RAFT, "[%zu] Recording seqnums\n", proxy->c->thread_id);
                // followers should also be recording in the bitmap?
                batch->record_seqnums();

                if (raft_is_leader(proxy->raft)) {
                    debug_print(DEBUG_RAFT, "[%zu] I am the leader submitting to system batch with seq num: %lu\n",
                                proxy->c->thread_id, batch->seqnum);
        ...
        case EntryType::kSwitchToBackupSeq:
            {
                ...
                if (raft_is_leader(proxy->raft) && proxy->c->in_recovery) {
                    ...
                    LOG_RECOVERY("Have %u resps left to receive\n", resps);
                    if (resps == 0) {
        ...
        default: {
            printf("trying to commit type: %d\n", static_cast<int>(our_entry->type));
            print_entry(our_entry);
...
inline int __log_offer(
    ...
    switch (our_entry->type) {
        ...
        case EntryType::kSequenceNumber: {
            if (!raft_is_leader(proxy->raft)) {
                debug_print(DEBUG_RAFT, "In log_offer for kSequenceNumber%s\n", 
                        our_entry->type == EntryType::kSequenceNumber? "" : "Noncontig");
                ...
                for (int i = 0; i < our_entry->batch_size; i++) {
                    ...
                    if (our_entry->type == EntryType::kSequenceNumberNoncontig) {
                        client_mdata_nc_t *cmdata = reinterpret_cast<client_mdata_nc_t *>(
                                &our_entry->cmdata_buf);
                        op->populate(cmdata[i].reqtype,
                                     cmdata[i].client_id, 
                                     cmdata[i].client_reqid,
                                     proxy->proxy_id, nullptr, 0, proxy, proxy->c);
                        memcpy(op->entry_val, cmdata[i].entry_val, MAX_CORFU_ENTRY_SIZE);
                    } else {
                        client_mdata_t *cmdata = reinterpret_cast<client_mdata_t *>(
                                &our_entry->cmdata_buf);
                        op->populate(cmdata[i].reqtype,
                                     cmdata[i].client_id, 
                                     cmdata[i].client_reqid,
                ...
                for (size_t i = 0; i < batch->batch_size(); i++) {
                    auto *op = batch->batch_client_ops[i];
                    proxy->client_retx_in_progress_map[op->client_id][op->client_reqid] = op;
                }
            }
            ...
            batch->seqnum = our_entry->seq_num;
...
int __log_pop(
    ...
    auto *wc = my_data->wc;
    Proxy *proxy = wc->proxies[my_data->pid];
    erpc::exit_assert(!raft_is_leader(proxy->raft), "I am leader popping an entry!?\n");
    free(entry_to_pop);
    return 0;
}
...
/** Callback for saving log entry changes.
 *
 * This callback is used for removing the first entry (eldest) from the log (ie. polling).
 *
...
{
    // set the callbacks
    raft_cbs_t *raft_callbacks = new raft_cbs_t;
