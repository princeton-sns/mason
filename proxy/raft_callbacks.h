    ...
    static inline void serialize(erpc::MsgBuffer &req_msgbuf, int node_id, int proxy_id,
        ...
        for (size_t i = 0; i < static_cast<size_t>(msg_ae->n_entries); i++) {
            ...
            *reinterpret_cast<msg_entry_t *>(buf) = msg_ae->entries[i];
            ...
            // Copy the entry data
            memcpy(buf, msg_ae->entries[i].data.buf, msg_ae->entries[i].data.len);
...
inline int __send_requestvote(
    ...
    auto *wc = my_data->wc;
    // todo segfault here, need unique proxy ids, can't index by proxy id anymore
...
inline Proxy *process_snapshot(WorkerContext *wc, snapshot_request_t *sr, Proxy *proxy) {
    ...
    proxy->raft = raft;
    wc->received_seqnums = proxy->received_seqnums; // only use with actual deserialization...
...
inline void send_snapshot_handler(erpc::ReqHandle *req_handle, void *_context) {
    ...
    if (sr->msgs_rcvd == 0) {
        ...
        sr->msgs = ss_req->total_msg_size + 1; // total_msg_size is the last index...
...
inline int __send_snapshot(
    ...
    static uint64_t freq_ghz = erpc::measure_rdtsc_freq();
