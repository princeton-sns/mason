...
typedef struct client_mdata {
    ReqType reqtype; // this should be removed when we move to arbitrary client ops
    uint16_t client_id;
...
public:
    ...
    // this is eventually arbitrary app arguments
    char entry_val[MAX_CORFU_ENTRY_SIZE];
...
public:
    ...
    void serialize(Archive & ar, const unsigned int) {
        ...
        // Operations in this batch
        ar & batch_client_ops; // vector to ClientOp *s
        ar & completed_ops;

        ar & acked_ops; // unordered map of ClientOps
        ar & nacked;
        ar & seqnum;

        ar & raft_entry_response;

        ar & has_seqnum; // only used on followers becoming leaders
    }
};
...
public:
    ...
    std::priority_queue<uint64_t, std::vector<uint64_t>, CmpBatchIds> done_batch_ids; // no pointers
    ...
    // calls raft_periodic if it has been at elast nms ms since the last call
...
public:
    ...
    // Recovery-related state
    Bitmap *received_seqnums; // needs to be serialized as per-proxy data =(
