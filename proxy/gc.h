...
// Fwd decls
uint64_t accumulate_bitmap_counts(uint64_t base_seqnum,
                                  uint64_t count,
...
initiate_garbage_collection(void *_c) {
  ...
  c->rpc->resize_msg_buffer(&(c->req_msgbuf),
                            GC_PAYLOAD_SIZE(nsequence_spaces));
  ...
  for (size_t i = 0; i < nsequence_spaces; i++) {
    ...
    while (bitmap_zero->counts[bitmap_zero->head_block] >= SEQNUMS_PER_BLOCK) {
      debug_print(DEBUG_GC,
                  "[%zu] truncating in initialization loop\n",
                  c->thread_id);
    ...
    if (count >= SEQNUMS_PER_BLOCK) {
      debug_print(DEBUG_GC,
                  "[%zu] Truncating when count >= SEQNUMS_PER_BLOCK on gc leader "
  ...
  c->avg_time_bw_trying_to_send =
      update_avg(erpc::rdtsc() - c->lt_tts,
                 c->avg_time_bw_trying_to_send,
                 c->tts_c);
  c->lt_tts = erpc::rdtsc();
  c->tts_c++;
  ...
  LOG_GC("[%zu] Initiating sending GC message to next proxy: "
         ...
         c->session_num_vec[static_cast<uint8_t>(MachineIdx::NEXTPX0)]);
  erpc::rt_assert(
      c->session_num_vec[static_cast<uint8_t>(MachineIdx::NEXTPX0)] != 0,
...
garbage_collection_handler(erpc::ReqHandle *req_handle, void *_context) {
  ...
  erpc::rt_assert(
      handler_msgbuf->get_data_size() == GC_PAYLOAD_SIZE(nsequence_spaces),
      "GC received wrong size message\n");

  if (!c->am_gc_leader()) {
    // there is only one proxy per thread... I don't think this is the only
    // place that takes advantage of this
    ...
    if (c->session_num_vec[static_cast<uint8_t>(MachineIdx::NEXTPX0)] == 0
      ...
      LOG_WARN("[%zu] Not connected to next_proxy yet. "
               "Not forwarding GC packet.\n", c->thread_id);
    } else {
    ...
    erpc::rt_assert(c->lt_s != 0,
                    "last time sent was 0 in ring completion!?\n");
    c->avg_ring_duration =
...
garbage_collection_cont_func(void *_c, void *_idx) {
  ...
  switch (*idx) {
    case 0:c->received_gc_response0 = true;
      break;
    case 1:c->received_gc_response1 = true;
      break;
    case 2:c->received_gc_response2 = true;
      break;
  }
}

uint64_t
accumulate_bitmap_counts(uint64_t base_seqnum,
                         uint64_t count,
                         uint64_t sequence_space) {
  for (auto &context : *context_vector) {
    ...
    if (base_seqnum == bitmap->base_seqnum) {
      ...
      if (count >= SEQNUMS_PER_BLOCK) {
        debug_print(DEBUG_GC,
                    "Context %d Truncating when base_seqnum "
                    "is equal, local count is %zu total count is %zu\n",
                    context->thread_id,
                    bitmap->counts[bitmap->head_block],
                    count);
      ...
      debug_print(DEBUG_GC,
                  "Context %d This context's base_seqnum is higher than the"
      ...
      // Safe to truncate
      debug_print(DEBUG_GC,
                  "Context %d Truncating when base_seqnums "
